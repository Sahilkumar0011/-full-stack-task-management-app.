...
const MenuPage = () => {
  ...
  const fetchMenu = async () => {
    try {
      ...
      const response = await axios.get('https://full-stack-task-management-app-1-1.onrender.com/api/menu', {
  ...
  const handleAddMenuItem = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('https://full-stack-task-management-app-1-1.onrender.com/api/menu', newItem, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchMenu();
    } catch (error) {
